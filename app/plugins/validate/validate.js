            ...
            var configMap = $.extend({
                ...
                validMethod = {
                    rules : {
                        ...
                        'radio' : {
                            validate : function(obj, callback, flag){
                                ...
                                var selRadio = $('input[name='+name+']:checked');
                                var wrongmsg = obj.attr('valid-wrong');
                                if(selRadio.length>0){
                                    return true;
                                }else{
                                    validMethod.showError(false,obj,wrongmsg,false);
                                    return false;
                                }
                            }
                        },
                        ...
                        'select': {
                            validate:function(obj, callback, flag){
                                ...
                                if( inputTxt>0 ){
                                    validateResult = true;
                                    isCanSubmit = true;
                                    validMethod.showError(validateResult,obj,wrongmsg,false);
                                }else{
                                    validateResult = false;
                                    isCanSubmit = false;
                                    validMethod.showError(validateResult,obj,wrongmsg,true);
                    ...
                    showError : function(flag,obj,str,focus){
                        if( flag == false ){
                            obj.closest('.inputframe').addClass('wrong font-wong');
                            if( $('.wrongmsg').length == 1 ){
                                ...
                                if(wrongmsg.length == 1){
                                    wrongmsg.html(str);
                                }else if( wrongmsg.length > 1 ){
                                    obj.parents('.inputframe').siblings('.wrongmsg').html(str);
                                } else {
                            ...
                            obj.closest('.inputframe').removeClass('wrong').removeClass('font-wong');
                            $('.wrongmsg').html('');
                        }
                    },
                    // 检测内容是否正确
                    _check : function(obj){
                        ...
                        var curVal = obj.val();
                        //只有包含了valid-empty的输入框才是必填的
                        //如果值为空，直接返货false，不用再走其他的判断
                        // console.log(curVal.length);
                        // console.log(obj);
                        if( curVal.length <=0 ){
                            if(data_require !== '' && data_require != undefined){
                        ...
                        $(document).ajaxComplete(function(event, xhr, settings) {
                            ...
                            if(!(inputs.lenght == inputs2.length)){
                                ...
                                inputs.each(function(){
                                    // $(this).unbind('change');
                                    $(this).live('change',function(){
                                        $(this).parent('.inputframe ').removeClass('wrong font-wong');
                                        $('.wrongmsg').html('');
                                        // if( $('.wrongmsg').length == 1 ){
                                        //     $('.wrongmsg').html('');
                                        // }else{
                         ...
                         */
                         inputs.each(function(){
                            if($(this).is('select')){
                                $(this).live('change',function(){
                                    validMethod.validate_select($(this), callback);
                                    $(this).removeClass('wrong').parent('.inputframe ').removeClass('wrong');
                                    $('.wrongmsg').html('');
                                });
                            }
                        });
                            ...
                            if( _flag && !$(this).hasClass('posting') ){
                                ...
                                if( ajax_url != undefined ){
                                    ...
                                    $.ajax({
                                        ...
                                        url : ajax_url,
                                        success : function(data){
                                            if(data.api_code == 1 || data.state == 1){
                                                if(ajax_success){
