            ...
            var configMap = $.extend({
                ...
                validMethod = {
                    rules : {
                        'check' : {
                            validate:function(obj, callback, flag){
                                ...
                                if(value != type){
                                    ...
                                    validMethod.showError(true,obj);
                                    return true;
                                }
                            }
                        },
                        ...
                        'Mphone': {
                            validate:function(obj, callback, flag){
                                ...
                                if( regExpressMobile.test(inputTxt) || regExpressPhone.test(inputTxt) || regExpressPhone2.test(inputTxt) ){
                                    validateResult = true;
                                    isCanSubmit = true;
                                }else{
                                    validateResult = false;
                    ...
                    showError : function(flag,obj,str,focus){
                        if( flag == false ){
                            ...
                            if( $('.wrongmsg').length == 1 ){
                                $('.wrongmsg').html(str).addClass('error');
                            }else{
                                var wrongmsg = obj.parents('dd,dt').find('.wrongmsg');
                                if(wrongmsg.length > 0){
                                    wrongmsg.html(str).addClass('error');
                                } else {
                                    obj.closest('.inputsection').find('.wrongmsg').html(str).addClass('error');
                                }
                            }
                            if( focus ){
                                $(obj).focus();
                            ...
                            if( $('.wrongmsg').length == 1 ){
                                $('.wrongmsg').html(' ').removeClass('error');
                            }else{
                                obj.parents('dd,dt').find('.wrongmsg').html(' ').removeClass('error');
                            }
                            obj.parents('.inputframe ').removeClass('wrong');
                            $('.wrongmsg').html('').removeClass('error');
                        }
                    },
                    // 检测内容是否正确
                    _check : function(obj){
                        ...
                        if(data_needcheck){
                            var data_needcheck_return = eval(data_needcheck+"()");
                                ...
                                $.ajax({
                                    ...
                                    success : function(data){
                                        if(data.api_code == 1){
                                            ...
                                            validateResult = false;
                                            isCanSubmit = false;
                                            validMethod.showError(validateResult,obj,data.message,true);
...
//                                            }
                            ...
                            if(validMethod.rules.hasOwnProperty(data_type)){
                                return validMethod.rules[data_type].validate(obj);
                            } else {
                        ...
                        configMap.targetObj.live(configMap.subEvent, function(){
                            ...
                            if( _flag && !$(this).hasClass('posting') ){
                                ...
                                if( ajax_url != undefined ){
                                    ...
                                    var ajax_response = validateObj.attr('ajax_response');
