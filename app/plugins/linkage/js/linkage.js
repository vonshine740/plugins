...
;(function($){
     $.fn.linkage = function(usersetting, callback){
        ...
        var linkageChild = $(this).find(config.child),
            ...
            objNext = null,
            self = this,
            linkType = linkageChild.length > 0? linkageChild[0].nodeName.toLowerCase():' ';
                            ...
                            var data = ajax({
                                ...
                                if(data){
                                    initData({
                                        ...
                                        $(self).find('select[data-name='+nextNext+']').html('<option value="-1" selected="selected">请选择</option>');
                                    });
                                }
                            });
                        }else if(dataNext && thisval <= 0){
                            var nextSel = $(self).find('select[data-name='+dataNext+']');
                            nextSel.html('<option value="0" selected="selected">请选择</option>');
                            var nextNext = nextSel.attr('data-next');
                            if(nextNext){
                                var nextNextSel = $(self).find('select[data-name='+nextNext+']');
